...
export async function POST() {
  return new Promise(async (resolve) => {
    // Check if Screenpipe is already running (existing code)
    try {
    ...
    // Start Screenpipe (existing code)
    ...
    const checkHealth = async () => {
      if (attempts >= maxAttempts) {
        return resolve(NextResponse.json({
          success: false,
          error: "Failed to start Screenpipe session after multiple attempts"
        }, { status: 500 }));
      }
      ...
      try {
        ...
        if (healthResponse.ok) {
          ...
          return resolve(NextResponse.json({
            ...
            health: healthData,
            message: "Screenpipe session started"
          }));
        }
      } catch (error) {
